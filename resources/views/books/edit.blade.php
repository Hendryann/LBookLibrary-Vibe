...
@section('title', 'Edit Book — Bibliotheca')

@section('content')
<div class="mb-4">
    <a href="{{ route('books.show', $book->id) }}" class="text-sm text-indigo-600 hover:underline">← Back to Book</a>
</div>
    ...
    <form method="POST" action="{{ route('books.update', $book->id) }}" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">{{ old('description', $book->description) }}</textarea>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('books.show', $book->id) }}" class="px-6 py-2 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200">
                Cancel
            </a>
        </div>
    </form>
