...
@section('title', 'Add Author — Bibliotheca')
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Add Author</h1>
    ...
    <form method="POST" action="{{ route('authors.store') }}" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="biography" rows="5"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">{{ old('biography') }}</textarea>
        </div>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('authors.index') }}" class="px-6 py-2 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200">
                Cancel
            </a>
        </div>
    </form>
