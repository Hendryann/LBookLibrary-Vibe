...
@section('title', 'Edit Category — Bibliotheca')

@section('content')
<div class="mb-4">
    <a href="{{ route('categories.show', $category->id) }}" class="text-sm text-indigo-600 hover:underline">← Back to Category</a>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 max-w-lg mx-auto">
    ...
    <form method="POST" action="{{ route('categories.update', $category->id) }}" class="space-y-5">
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('categories.show', $category->id) }}" class="px-6 py-2 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200">
                Cancel
            </a>
        </div>
    </form>
